...
int main (){
    ...
    while ( abs ( att_est . phi ) <= pi /4.0 f && abs ( att_est . theta ) <= pi /4.0 f && abs (
    att_est .p) <= 4.0f*pi && abs( att_est .q) <= 4.0 f*pi && abs ( att_est .r) <= 4.0
    f*pi){
        ...
        if ( flag )
        {
            ...
            mixer . actuate (f_t ,0.0f, att_cont . tau_theta ,0.0 f);
            }
    }
